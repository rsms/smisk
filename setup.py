...
from distutils.command.clean import clean as builtin_clean
...
# This is popluated by a TestRunner after the "test" command has been run
test_result = None
...
def revision_from_version_h():
  ...
  try:
    for line in f:
      #define SMISK_BUILD_ID "DATETIME-REVISION"<LF>
      if line[:17] == '#define SMISK_BUILD_ID':
...
repo_has_changed = False
if not os.path.exists('src/version.h'):
  repo_has_changed = True
elif os.path.getmtime('src/version.h') < os.path.getmtime('.hg') \
...
class smisk_clean(builtin_clean):
  ...
  def run(self):
    builtin_clean.run(self)
    from glob import iglob
    
    print 'Removing files generated by setup.py'
    for path in ['MANIFEST', '_config.py', 'src/version.h', 'src/system_config.h']:
      self.remove_file(path)
    
    print 'Removing logfiles from example applications'
    for path in iglob('examples/*/*.log'):
      self.remove_file(path)
